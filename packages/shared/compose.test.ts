...
import type { PromiseFn } from './async'
import { asyncTimeout, sleep, withRetry } from './async'
import { FAILURE, SUCCESS, TIMEOUT } from './constant'
...
describe('compose', () => {
  ...
  it('throws at runtime if argument is not a function', () => {
      ...
      expect(() =>
        compose(square, add, Number.NaN as unknown as sFunc)(1, 2),
      ).toThrow()
      expect(() =>
...
describe('compose timeout or retry', () => {
  it('错误，全部错误，默认3次重试', async () => {
    ...
    await expect(connectWithAsyncAbilities()).rejects.toThrow(FAILURE)
...
describe('compose in class', () => {
  ...
  it('装饰器实现,全部失败', async () => {
    ...
    const t = new Test()
    await expect(t.connect()).rejects.toThrow(FAILURE)
    expect(mockFn).toHaveBeenCalledTimes(4)
  })
